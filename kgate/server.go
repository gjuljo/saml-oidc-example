...
func (r *oauthProxy) createUpstreamProxy(upstream *url.URL) error {
	...
	proxy.OnResponse().DoFunc(func(resp *http.Response, ctx *goproxy.ProxyCtx) *http.Response {
		if resp != nil {
			// accessToken := resp.Request.Context().Value("accessToken")
			scope := resp.Request.Context().Value(contextScopeName).(*RequestScope)
			if scope != nil && scope.Identity.updated {
				r.log.Debug("updating access token cookie")
				accessToken := scope.Identity.token.Encode()
				domain := ""
				...
				resp.Header.Add("Set-Cookie", "kc-access="+accessToken+"; Path=/; Domain="+domain+"; HttpOnly")
			} else {
				r.log.Debug("access token still valid, no need to update it")
			}
		}
